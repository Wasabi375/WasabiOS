...
impl OvmfFileType {
    ...
    pub fn get_user_provided_path(self, opts: &QemuConfig) -> Option<PathBuf> {
        let (path, var_name) = match self {
            Self::Code => (opts.uefi.ovmf_code, ENV_VAR_OVMF_CODE),
            Self::Vars => (opts.uefi.ovmf_vars, ENV_VAR_OVMF_VARS),
            Self::Shell => (None, ENV_VAR_OVMF_SHELL),
        };
        path.map(|p| p.to_path_buf())
            .or_else(|| env::var_os(var_name).map(PathBuf::from))
    }
}
...
impl OvmfPaths {
    ...
    pub fn find_ovmf_file(file_type: OvmfFileType, opts: &QemuConfig) -> Result<PathBuf> {
        if let Some(path) = file_type.get_user_provided_path(opts) {
    ...
    /// [`Self::find_ovmf_file`].
    pub fn find(opts: &QemuConfig) -> Result<Self> {
        let code = Self::find_ovmf_file(OvmfFileType::Code, opts)?;
        let vars = Self::find_ovmf_file(OvmfFileType::Vars, opts)?;
