...
const MAX_STYLE_COUNT: usize = STYLES.len();
...
impl Styles {
    ...
    fn from_u8(u: u8) -> Option<StaticVec<Styles, MAX_STYLE_COUNT>> {
        if u == CLEARV {
            return None;
        }

        let res: StaticVec<Styles, MAX_STYLE_COUNT> = STYLES
            .iter()
...
impl Style {
    ...
    pub(crate) const TO_STR_MAX_LEN: usize = (Styles::TO_STR_MAX_LEN + 1) * MAX_STYLE_COUNT;

    pub(crate) fn to_str(self) -> StaticString<{ Self::TO_STR_MAX_LEN }> {
        let styles = Styles::from_u8(self.0).unwrap_or_default();
        StaticString::join_from(styles.iter().map(|s| s.to_str()), ";")
    }
