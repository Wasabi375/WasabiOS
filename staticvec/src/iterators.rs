        ...
        if n >= distance_between(self.end, self.start) {
            ...
            unsafe {
                match size_of::<T>() {
                    0 => {
                        self.end =
                            (self.end as *const u8).wrapping_offset(-(n as isize)) as *const T;
        ...
        match self.end - self.start {
            ...
            _ => {
                let res = match size_of::<T>() {
                    0 => Some(unsafe {
                        zst_ptr_add(StaticVec::first_ptr(&self.data), self.start).read()
                    }),
    ...
    fn next_back(&mut self) -> Option<T> {
        match self.end - self.start {
            ...
            _ => {
                ...
                match size_of::<T>() {
                    0 => Some(unsafe {
                        zst_ptr_add(StaticVec::first_ptr(&self.data), self.end).read()
                    }),
